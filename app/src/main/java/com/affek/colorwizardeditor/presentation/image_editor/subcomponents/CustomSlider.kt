...
private fun SliderImpl(
    ...
    val rawOffset = remember { mutableStateOf(-1f) }
    ...
    val draggableState = remember(valueRange) {
        SliderDraggableState {
            val maxPx = max(totalWidth.value - thumbWidth.value / 2f, 0f)
            val minPx = min(thumbWidth.value / 2f, maxPx)
    ...
    Layout(
        ...
        rawOffset.value = if(rawOffset.value == -1f) totalWidth.value * realBasePointPositionFraction else rawOffset.value
        ...
        if (enabled) {
            ...
            pointerInput(draggableState, interactionSource, maxPx, isRtl) {
                detectTapGestures(
                    onDoubleTap = {
                        scope.launch {
                            animateToTarget(draggableState, rawOffset.value, (maxPx-2*minPx) * realBasePointPositionFraction + minPx)
                            gestureEndAction.value.invoke()
                        }
                    }
                )
