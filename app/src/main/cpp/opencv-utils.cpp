...
void colorTransferIntensity(Mat& source, Mat& color, float intensity, float exposureCompensation) {
    ...
    addWeighted(source, 1, diff, intensity, exposureCompensation, source, CV_16S);
    source.convertTo(source, CV_8UC4);

}

void colorEdit(Mat& source, float saturation) {
//    cvtColor(source, source, COLOR_RGB2HLS);
//    Mat channels_hsv[3];
//    split(source, channels_hsv);
//    add(channels_hsv[2], saturation, channels_hsv[2]);
//    merge(channels_hsv, 3, source);
//    cvtColor(source, source, COLOR_HLS2RGB);
    Mat gray;
    cvtColor(source, gray, COLOR_RGB2GRAY);
    cvtColor(gray, gray, COLOR_GRAY2RGBA);
    addWeighted(source, saturation, gray, 1.0 - saturation, 0.0, source);
}
